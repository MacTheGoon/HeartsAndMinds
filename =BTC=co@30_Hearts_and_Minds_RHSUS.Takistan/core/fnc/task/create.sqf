...
switch (_task_id) do {
    case "0" : {
        _description = [localize "STR_BTC_HAM_MISSION_DEFEAT_DESC", localize "STR_BTC_HAM_MISSION_DEFEAT_TITLE", localize "STR_BTC_HAM_MISSION_DEFEAT_TITLE"]; //"Defeat ISIS once and for all","Defeat ISIS","Defeat ISIS"
        _type = "kill";
    };
    case "1" : {
        _description = [localize "STR_BTC_HAM_MISSION_DESTORY_DESC", localize "STR_BTC_HAM_MISSION_DESTORY_TITLE", localize "STR_BTC_HAM_MISSION_DESTORY_TITLE"]; //"Destroy all the hideouts of ISIS","Destroy all the hideouts","Destroy all the hideouts"
        _type = "destroy";
    };
    case "2" : {
        _description = [localize "STR_BTC_HAM_MISSION_SEIZE_DESC", localize "STR_BTC_HAM_MISSION_SEIZE_TITLE", localize "STR_BTC_HAM_MISSION_SEIZE_MRK"]; //"Seize the last positions held by ISIS fighters","Seize the last ISIS positions","Seize the last ISIS fighters positions"
        _type = "move";
    };
    case "3" : {
    ...
    case "6" : {
        _description = [format [localize "STR_BTC_HAM_SIDE_CONQUER_DESC", _location], format [localize "STR_BTC_HAM_SIDE_CONQUER_TITLE", _location], format [localize "STR_BTC_HAM_SIDE_CONQUER_TITLE", _location]]; //"%1 has been conquered by ISIS! Local population is being terrorized, they are asking for help!", _location], "Free " + _location, "Free " + _location)
        _type = "attack";
    };
    case "7" : {
        _description = [format [localize "STR_BTC_HAM_SIDE_TOWER_DESC", _location], format [localize "STR_BTC_HAM_SIDE_TOWER_TITLE", _location], format [localize "STR_BTC_HAM_SIDE_TOWER_TITLE", _location]]; //"A ISIS communications tower has been located in %1. Local population is asking for your help to destroy it! (Use one M183 explosive satchel)", _location], "Destroy tower in " + _location, "Destroy tower in " + _location)
        _type = "destroy";
    };
    case "8" : {
    ...
    case "12" : {
        _description = [format [localize "STR_BTC_HAM_SIDE_CONVOY_DESC", _location], format [localize "STR_BTC_HAM_SIDE_CONVOY_TITLE", _location], format [localize "STR_BTC_HAM_SIDE_CONVOY_TITLE", _location]]; //"An armed ISIS convoy is going to attack %1. Local population is asking for your help to destroy it before it gets there!", _location], "Destroy ISIS convoy attacking " + _location, "Destroy ISIS convoy attacking " + _location)
        _type = "attack";
    };
    case "13" : {
